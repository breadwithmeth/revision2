...
export async function onecAckRoute(fastify: FastifyInstance) {
  fastify.post('/onec/inventory-documents/:id/ack', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const result = await InventoryService.ack(id);
